...
function DashboardPollItemWithBlock({ poll }: { poll: Poll }) {
      ...
      >
        <div className="">
          <Link href={`/polls/${poll.public_link}`}>
            <h4 className="text-primary cursor-pointer font-bold text-2xl">
              {poll.title}
              {"  "}
              {poll.reportCount && poll.reportCount > 0 && (
                <span className="text-sm text-red-500">
                  {poll.reportCount}{" "}
                  {poll.reportCount === 1 ? "report" : "reports"}
                </span>
              )}
            </h4>
          </Link>
        ...
        <div className="flex md:flex-col gap-2 items-end md:items-start">
          ...
          <p className="text-xl text-primary text-end">
            {poll.views.toLocaleString()} {poll.views === 1 ? "view" : "views"}
          </p>
          <p className="text-md text-primary text-end md:hidden">/</p>
          <p className="text-xl text-primary text-end">0 shares</p>
        </div>
        <div className="flex gap-6 justify-center">
          <div className="flex md:flex-col flex-row gap-2 items-center justify-center">
            ...
            >
              {poll.isBlocked ? "unblock poll" : "block poll"}
            </CommonButton>
          </div>
...
export default DashboardPollItemWithBlock;
